...
const JP_BG = '宝石アクセサリー美しい輝きジュエリー愛装飾品桜ストア宝石アクセサリー美しい輝きジュエリー'
...
export default function CatalogPage() {
  ...
  const s = {
    page: { minHeight: '100vh', background: '#0a0a0a', position: 'relative', overflowX: 'hidden' },
    jpBg: {
      position: 'fixed',
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        <div style={s.catsWrap}>
          <CatButton active={activeCat === 'all'} onClick={() => setActiveCat('all')} label="Todos" jp="全部" />
          {categories.map(cat => (
            <CatButton
              key={cat.id}
...
function CatButton({ active, onClick, label, jp }) {
  ...
  return (
    <button style={s.btn} onClick={onClick}
      onMouseEnter={e => { if (!active) { e.currentTarget.style.borderColor = '#e8609a'; e.currentTarget.style.color = '#e8609a' } }}
      onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = '#2a1a22'; e.currentTarget.style.color = '#888' } }}
    >
      {label}
